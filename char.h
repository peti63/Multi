...
enum EPointTypes
{
	...
	POINT_INVENTORY,
	POINT_SKILL_INVENTORY,
	POINT_MATERIAL_INVENTORY,
	POINT_STONE_INVENTORY,
	POINT_CHEST_INVENTORY,
	POINT_ENCHANT_INVENTORY,
	POINT_COSTUME_INVENTORY,
#endif
	// don't touch, here it is min wep etc for client
#ifdef NEW_STATISTICS
	POINT_TYPE_REFINE = 205,
	POINT_TYPE_CRAFT,
	POINT_TYPE_FISH,
	POINT_TYPE_DUNGEONS,
	POINT_TYPE_SHOUT,
	POINT_TYPE_ADD_SOCKET,
	POINT_TYPE_CHESTS,
	POINT_TYPE_TRADE,
	POINT_TYPE_MONSTERS,
	POINT_TYPE_BOSSES,
	...
	POINT_TYPE_DUEL,
	POINT_TYPE_DEAD,
	POINT_TYPE_DMG_STONES,
	POINT_TYPE_DMG_HIT_PC,
	POINT_TYPE_DMG_SKILL_PC,
	POINT_TYPE_GUILD_WAR,
	POINT_TYPE_GUILD_KILLS,
	POINT_TYPE_GUILD_FLAGS,
	POINT_TYPE_BATTLEPASS,
	POINT_TYPE_SKILLBOOKS,
	POINT_TYPE_HUNTING_MISSIONS,
#endif
	POINT_MAX
};
...
class CHARACTER : public CEntity, public CFSM, public CHorseRider
{
	...
	public:
		...
		bool IsHydraMonsters()
		{
			...
			if (GetRaceNum() >= 3401 && GetRaceNum() <= 3605)
				return true;
		
			if (GetRaceNum() >= 3950 && GetRaceNum() <= 3959)
				return true;
			return false;
		};
		...
		WORD				GetMobAttackRange(DWORD victimRace = 0) const;
	...
	public:
		...
		bool			WarpSet(long x, long y, long lRealMapIndex = 0, BYTE newChannel = 0);
	...
	public:
		void			SyncQuickslot(DWORD bType, DWORD bOldPos, DWORD bNewPos);
		bool			GetQuickslot(DWORD pos, TQuickslot ** ppSlot);
		bool			SetQuickslot(DWORD pos, TQuickslot & rSlot);
		bool			DelQuickslot(DWORD pos);
		bool			SwapQuickslot(DWORD a, DWORD b);
		void			ChainQuickslotItem(LPITEM pItem, DWORD bType, DWORD bOldPos);
	...
	public:
		...
		void			ProcessSortedItem(LPITEM item);
		...
		BYTE			DetermineInventoryType(BYTE type);
		void			CollectItems(BYTE type, std::vector<LPITEM>& withSocket, std::vector<LPITEM>& withoutSocket);
		void			GetInventoryRange(BYTE type, WORD& start, WORD& end);
		LPITEM			GetSpecialInventoryItem(BYTE type, WORD index);
		void			ProcessSortedItems(const std::vector<LPITEM>& items, BYTE inventory_type);
		...
		LPITEM			GetSwitchInventoryItem(WORD wCell) const;
		LPITEM			GetCostumeInventoryItem(WORD wCell) const;
#endif

		void			SetWear(WORD bCell, LPITEM item);
		LPITEM			GetWear(WORD bCell) const;
		...
		bool			UseItemEx(LPITEM item, TItemPos DestCell = NPOS);
		bool			UseItem(TItemPos Cell, TItemPos DestCell = NPOS);
		bool			UseItemStack(TItemPos Cell, int count = 1);
		...
		bool			RefineInformation(WORD bCell, BYTE bType, int iAdditionalCell = -1, int RefinementType = 0);
		...
		LPITEM			AutoGiveItemType(DWORD dwItemVnum, BYTE type, DWORD bCount=1, int iRarePct = -1, bool bMsg = true);
		void			AutoGiveItem(LPITEM item, bool longOwnerShip = false);
		void			AutoGiveItemType(LPITEM item, BYTE type, bool longOwnerShip = false);
		...
		int				GetEmptyGiftBoxInventoryFromIndex(WORD index, BYTE itemSize) const; //SPLIT ITEMS
		int				GetEmptySwitchInventoryFromIndex(WORD index, BYTE itemSize) const; //SPLIT ITEMS
		int				GetEmptyCostumeInventoryFromIndex(WORD index, BYTE itemSize) const; //SPLIT ITEMS
		...
		int				GetEmptySwitchInventory(BYTE size) const;
		int				GetEmptyCostumeInventory(BYTE size) const;
#endif
		int				CountEmptyInventory() const;
		int 			CountEmptySpecialInventory(BYTE type) const;
...
#ifdef ENABLE_EXTEND_INVEN_SYSTEM
		int				Inventory_Point() const { return m_points.iNormalInventory; }
		int				Inventory_Size() const { return INVENTORY_OPEN_PAGE_SIZE + (INVENTORY_WIDTH*Inventory_Point()); }
		void			Set_Inventory_Point(INT inventory) { m_points.iNormalInventory = inventory; }


		int				Skill_Inventory_Point() const { return m_points.iSkillbookInventory; }
		int				Skill_Inventory_Size() const { return SPECIAL_INVENTORY_OPEN_PAGE_SIZE + (INVENTORY_WIDTH*Skill_Inventory_Point()); }
		void			Set_Skill_Inventory_Point(INT inventory) { m_points.iSkillbookInventory = inventory; }

		int				Material_Inventory_Point() const { return m_points.iMaterialInventory; }
		int				Material_Inventory_Size() const { return SPECIAL_INVENTORY_OPEN_PAGE_SIZE + (INVENTORY_WIDTH*Material_Inventory_Point()); }
		void			Set_Material_Inventory_Point(INT inventory) { m_points.iMaterialInventory = inventory; }

		int				Stone_Inventory_Point() const { return m_points.iStoneInventory; }
		int				Stone_Inventory_Size() const { return SPECIAL_INVENTORY_OPEN_PAGE_SIZE + (INVENTORY_WIDTH*Stone_Inventory_Point()); }
		void			Set_Stone_Inventory_Point(INT inventory) { m_points.iStoneInventory = inventory; }

		int				Chest_Inventory_Point() const { return m_points.iChestInventory; }
		int				Chest_Inventory_Size() const { return SPECIAL_INVENTORY_OPEN_PAGE_SIZE + (INVENTORY_WIDTH*Chest_Inventory_Point()); }
		void			Set_Chest_Inventory_Point(INT inventory) { m_points.iChestInventory = inventory; }

		int				Enchant_Inventory_Point() const { return m_points.iEnchantInventory; }
		int				Enchant_Inventory_Size() const { return SPECIAL_INVENTORY_OPEN_PAGE_SIZE + (INVENTORY_WIDTH*Enchant_Inventory_Point()); }
		void			Set_Enchant_Inventory_Point(INT inventory) { m_points.iEnchantInventory = inventory; }

		int				Costume_Inventory_Point() const { return m_points.iCostumeInventory; }
		int				Costume_Inventory_Size() const { return SPECIAL_INVENTORY_OPEN_PAGE_SIZE + (INVENTORY_WIDTH*Costume_Inventory_Point()); }
		void			Set_Costume_Inventory_Point(INT inventory) { m_points.iCostumeInventory = inventory; }
		bool			Update_Inventory(INT type);
#endif
	...
	public:
		...
		void				SetNormalName(const char* newName);
		void				SetMountName(const char* newName);
	...
	public:
		...
		bool	IsOpenMall() const { return GetMall() ? true : false; }
	...
	public:
		void SetLastSyncTime(const std::chrono::steady_clock::time_point& timePoint) { m_lastSyncTime = timePoint; }
		const std::chrono::steady_clock::time_point& GetLastSyncTime() const { return m_lastSyncTime; }
	...
	public:
		...
		LPOFFLINESHOP		GetOfflineShopPrevent() const { return m_pkOfflineShop; }

		bool				IsOfflineShopNPC() { return (IsNPC() && GetOfflineShop()); }

		bool				GetOfflineShopPanel() {return isOfflineShopPanelOpen;}
		bool				GetOfflineShopPanelPrevent() const {return isOfflineShopPanelOpen;}
		void				SetOfflineShopPanel(bool flag) {isOfflineShopPanelOpen= flag;}

		bool				GetOfflineShopRefresh() {return isOfflineShopRefresh;}
		void				SetOfflineShopRefresh(bool flag) {isOfflineShopRefresh = flag;}
		...
		bool				CanSearch();
		bool				IsLookingSearchItem(DWORD itemID, bool remove = false, bool sendCommand = false);
		void				SetLookingSearch(const std::vector<DWORD> dataVector);
		void				GetPageItems(int pageIdx, std::vector<DWORD>& m_ItemList);
		int					GetLookingItemCount() { return m_vecSearchLooking.size(); }
...
#ifdef __SASH_SKIN__
	public:
		bool			UpgradeDreamSoul(LPITEM item, bool printMsg = true);
#endif
...
#ifdef NEW_STATISTICS
	public:
		void	SetPointStatistics(BYTE typeIndex, int64_t value);
		void	SetStatistics(BYTE typeIndex, int64_t value);
		int64_t	GetStatistics(BYTE missionIndex);

	protected:
		std::map<BYTE, int64_t> m_statsData;
#endif
};
