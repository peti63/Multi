...
static bool HWIDBan(DWORD dwAID)
{
    ...
    snprintf(szQuery, sizeof(szQuery), "SELECT `hwid`, `old_hwid` FROM `account`.`hwid` WHERE `account_id` = %u", dwAID);
    std::unique_ptr<SQLMsg> pMsg1(DBManager::instance().DirectQuery(szQuery));
    MYSQL_RES* pRes1 = pMsg1->Get()->pSQLResult;

    if (pRes1 && mysql_num_rows(pRes1) > 0)
    {
        MYSQL_ROW row = mysql_fetch_row(pRes1);
        DWORD dwHWID = strtoul(row[0], nullptr, 10);
        DWORD dwOldHWID = strtoul(row[1], nullptr, 10);

        snprintf(szQuery, sizeof(szQuery), "UPDATE `account`.`account` SET `status` = 'BLOCK' WHERE `id` IN (SELECT `account_id` FROM `account`.`hwid` WHERE `hwid` = %u OR `old_hwid` = %u)", dwHWID, dwOldHWID);
        std::unique_ptr<SQLMsg> pMsg2(DBManager::instance().DirectQuery(szQuery));

        snprintf(szQuery, sizeof(szQuery), "UPDATE `account`.`hwid` SET `ban` = 1 WHERE `hwid` = %u OR `old_hwid` = %u", dwHWID, dwOldHWID);
        std::unique_ptr<SQLMsg> pMsg3(DBManager::instance().DirectQuery(szQuery));

        if (pMsg2->Get()->uiAffectedRows > 0 && pMsg3->Get()->uiAffectedRows > 0)
            return true;
    }
...
ACMD(do_ban)
{
	...
	split_argument(argument, vecArgs);
	if (vecArgs.size() < 2) { 
		ch->ChatPacket(CHAT_TYPE_INFO, "Wrong syntax, usage: /ban player time .(time in hours! eg: 1 = 1 hour)");
		return;
	}

	const char* szName = vecArgs[1].c_str();
	int iDuration;
	...
	if (!str_to_number(iDuration, vecArgs[2].c_str()))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Wrong syntax, usage: /ban player time . (time in hours! eg: 1 = 1 hour)");
		return;
	}
	...
	if (tch->GetGMLevel() > GM_PLAYER && ch->GetGMLevel() < GM_HIGH_WIZARD)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Do not ban GMs.");
		return;
	}

	auto msg(DBManager::instance().DirectQuery("UPDATE account.account SET availDt = FROM_UNIXTIME(UNIX_TIMESTAMP(CURRENT_TIMESTAMP()) + %i) WHERE id = %d", iDuration * 3600, tch->GetDesc()->GetAccountTable().id));
	
	tch->GetDesc()->DelayedDisconnect(5);

	sys_log(0, "%s[%d] banned %s for %i hours", ch->GetName(), ch->GetPlayerID(), szName, iDuration);

	ch->ChatPacket(CHAT_TYPE_INFO, "%s has been banned for %i hours.", szName, iDuration);
...
void do_ipban(LPCHARACTER ch, const char *argument, int cmd, int subcmd)
{
	...
	if (d && d->GetCharacter()) {
		...
		if (d->GetCharacter()->IsGM() && ch->GetGMLevel() < GM_HIGH_WIZARD) {
...
ACMD(do_item)
{
	...
	if (item)
	{
		...
		{
			int iEmptyPos = ch->GetEmptySwitchInventory(item->GetSize());
		...
		{
			int iEmptyPos = ch->GetEmptyCostumeInventory(item->GetSize());
...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent)
	{
		...
		&& pkChr->GetRider() == NULL)
		{
			CNewMobTimer::Instance().Dead(pkChr, NULL);
...
ACMD(do_item_purge)
{
	...
	else if (!strArg.compare(0, 6, "sinv"))
	{
		for (BYTE byPage = 0; byPage < 6; ++byPage)
		{
			switch (byPage)
			{
			...
			case 4: // Switch inventory
			{
				for (i = 0; i < SPECIAL_INVENTORY_MAX_NUM; ++i)
				{
					if ((item = ch->GetSwitchInventoryItem(SWITCH_INVENTORY_SLOT_START + i)))
					{
						ITEM_MANAGER::instance().RemoveItem(item, "PURGE");
						ch->SyncQuickslot(QUICKSLOT_TYPE_ITEM, SWITCH_INVENTORY_SLOT_START + i, 255);
					}
				}
			}
			break;

			case 5: // Costume inventory
			{
				for (i = 0; i < SPECIAL_INVENTORY_MAX_NUM; ++i)
				{
					if ((item = ch->GetCostumeInventoryItem(COSTUME_INVENTORY_SLOT_START + i)))
					{
						ITEM_MANAGER::instance().RemoveItem(item, "PURGE");
						ch->SyncQuickslot(QUICKSLOT_TYPE_ITEM, COSTUME_INVENTORY_SLOT_START + i, 255);
					}
				}
			}
			break;
...
ACMD(do_who)
{
	auto c_set_desc = DESC_MANAGER::instance().GetClientSet();
	auto it = c_set_desc.begin();
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "There are currently %d players active on all open channels.", dwLoginCount);
}
...
typedef enum do_set_types_s {GOLD, GEM, RACE, SEX, JOB, EXP, MAX_HP, MAX_SP, SKILL, ALIGNMENT, ALIGN} do_set_types_t;
}
...
ACMD(do_block_chat)
{
	...
	if (lBlockDuration < 0)
	{
		if (ch)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Write something like this for time:");
		...
		case 'c':
			{
				...
				{
					ch->PointChange(POINT_GOLD, -(int64_t)t->dwPrice);
...
ACMD(do_get_item_id_list)
{
	for (int i = 0; i < INVENTORY_AND_EQUIP_SLOT_MAX; i++)
	{
		...
		if (item != NULL)
			ch->ChatPacket(CHAT_TYPE_INFO, "cell : %d, name : %s, id : %d", item->GetCell(), item->GetLocaleName(ch->GetLanguage()), item->GetID());
	}
}
...
ACMD(do_dragon_soul)
{
	...
	switch (arg1[0])
	{
		case 'a':
		{
			...
			if ((ch->IsStateMove() || bMoving) && ch->GetWarMap())
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("가만히 있을 때만 착용할 수 있습니다."));
				return;
			}
		...
		break;
#if defined(DRAGON_SOUL_REFINEMENT_FROM_WINDOW)
		case 'r':
			{
				if (ch->DragonSoul_IsQualified())
...
ACMD(do_test_accessory)
{
	...
	split_argument(argument, vecArgs);
	if (vecArgs.size() < 4) { 
		ch->ChatPacket(CHAT_TYPE_INFO, "/test_acc INV_SLOT_IDX SOCKET_IDX VALUE");
		return; 
	}
	...
	int dwSlotPos, dwSocketIdx, dwValue;
	if (!str_to_number(dwSlotPos, vecArgs[1].c_str()) || !str_to_number(dwSocketIdx, vecArgs[2].c_str()) || !str_to_number(dwValue, vecArgs[3].c_str())) {
		ch->ChatPacket(CHAT_TYPE_INFO, "/test_acc INV_SLOT_IDX SOCKET_IDX VALUE");
		return;
	}

	if (!(dwSocketIdx >= 1 && dwSocketIdx <= 3))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "please put right socket index! (1 - 3)");
		return;
	}
	...
	LPITEM item = ch->GetInventoryItem(dwSlotPos);
	if (item)
	{
		if (!item->IsAccessoryForSocket())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "This cannot be modify");
			return;
		}
		else if (item->GetSocket(dwSocketIdx - 1) <= 0)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "you can't change this socket index!");
			return;
		}

		item->SetSocket(dwSocketIdx - 1, (long)dwValue);
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "This slot idx not has any item for modify!");
		return;
	}
}
#endif
