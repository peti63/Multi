...
namespace quest
{
	ALUA(_get_locale)
	{
		...
		lua_pushstring(L, ch ? GetLocaleCodeName(ch->GetLanguage()) : GetLocaleCodeName(LOCALE_DEFAULT));
		return 1;
	}

	// locale_quest(arg1, arg2)
	// @arg1 <required> locale translation code.
	// @arg2 <optional> country code.
	ALUA(_locale_quest)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("QUEST locale_quest wrong argument.");
			return 0;
		}
		...
		if (lpChr != nullptr)
		{
			// NOTE: Support for getting other locale translations.
			// Used for arena_manager.quest, marriage_manage.quest and similar quests.
			BYTE bLocale = lpChr->GetLanguage();
			if (lua_isnumber(L, 2))
				bLocale = lua_tonumber(L, 2);

			lua_pushstring(L, LC_QUEST(lua_tonumber(L, 1), bLocale));
		}
		else
			lua_pushstring(L, "");
		return 1;
	...
	{
		DWORD dwMapIndex = lua_tonumber(L, 1);
		DWORD dwVID = lua_tonumber(L, 2);
		int64_t iMinHP = (int64_t)lua_tonumber(L, 3);
		int64_t iMaxHP = (int64_t)lua_tonumber(L, 4);

		FSendAlliancePacket f(dwVID, iMinHP, iMaxHP);
		LPSECTREE_MAP pSecMap = SECTREE_MANAGER::instance().GetMap(dwMapIndex);
		if (pSecMap)
			pSecMap->for_each(f);
		return 0;
	...
	int _item_name(lua_State* L)
	{
		if (lua_isnumber(L, 1))
		{
			...
			{
				LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
				lua_pushstring(L, LC_ITEM_NAME(dwVnum, ch ? ch->GetLanguage() : LOCALE_DEFAULT));
			}
			else
				lua_pushstring(L, "");
		}
	...
	int _mob_name(lua_State* L)
	{
		if (lua_isnumber(L, 1))
		{
			...
			{
				LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
				lua_pushstring(L, LC_MOB_NAME(dwVnum, ch ? ch->GetLanguage() : LOCALE_DEFAULT));
			}
			else
				lua_pushstring(L, "");
		}
	...
	ALUA(_skill_name)
	{
		if (lua_isnumber(L, 1))
		{
			...
			LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
			lua_pushstring(L, LC_SKILL_NAME(dwVnum, ch ? ch->GetLanguage() : LOCALE_DEFAULT));
		}
		else
			lua_pushstring(L, "");
...
#define NAN (INFINITY-INFINITY)
	...
	ALUA(_monthIndex)
	{
		...
		lua_pushnumber(L, vKey.tm_mon);
		return 1;
	}
	ALUA(_do_battlepass_mission)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (NULL != ch)
		{
			BYTE missionIndex = (BYTE)lua_tonumber(L, 1);
			DWORD missionSubValue = (DWORD)lua_tonumber(L, 2);
			long long missionValue = (int)lua_tonumber(L, 3);
			CHARACTER_MANAGER::Instance().DoMission(ch, missionIndex, missionValue, missionSubValue);
		}
		return 0;
	}
	ALUA(_clear_old_battlepass)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (NULL != ch)
	...
	void RegisterGlobalFunctionTable(lua_State* L)
	{
		...
		luaL_reg global_functions[] =
		{
			...
			{"do_battlepass_mission", _do_battlepass_mission },
			{"clear_old_battlepass", _clear_old_battlepass },
#endif
			{ "LC", _get_locale },
