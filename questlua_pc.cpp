...
namespace quest
{
	...
	ALUA(pc_is_select_bonus_uriel)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		lua_pushboolean(L, ch->FindAffect(AFFECT_SINGLE_BONUS_URIEL) ? true : false);
		return 1;
	}

	ALUA(pc_is_select_bonus_uriel_delay)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		lua_pushboolean(L, ch->FindAffect(AFFECT_SINGLE_BONUS_URIEL_DELAY) ? true : false);
		return 1;
	}

	ALUA(pc_is_pet_delay)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		lua_pushboolean(L, ch->FindAffect(AFFECT_NAME_PET) ? true : false);
		return 1;
	}

	ALUA(pc_is_mount_delay)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		lua_pushboolean(L, ch->FindAffect(AFFECT_NAME_MOUNT) ? true : false);
		return 1;
	}
	...
	ALUA(pc_give_or_drop_item_and_select)
	{
		...
		LPITEM item = ch->AutoGiveItemType(dwVnum, INVENTORY, icount);
	...
	ALUA(pc_get_wear)
	{
		...
		WORD bCell = (BYTE)lua_tonumber(L, 1);
...
teleport_area:
		...
		for ( int i=0; i < INVENTORY_MAX_NUM + WEAR_MAX_NUM; i++ )
		{
			...
			if ( pItem != NULL )
			{
				if ( pItem->IsEquipped() == false )
				{
					...
					{
						lua_pushstring( L, pItem->GetLocaleName(pChar->GetLanguage()));
						lua_rawseti( L, -2, 1 );
	...
	ALUA(pc_give_award_socket)
	{
		...
		DBManager::instance().Query("INSERT INTO item_award (login, vnum, count, given_time, why, mall, socket0, socket1, socket2, socket3, socket4, socket5, socket6, socket7)select '%s', %d, %d, now(), '%s', 1, %s, %s, %s, %s, %s, %s, %s, %s from DUAL where not exists (select login, why from item_award where login = '%s' and why  = '%s') ;",
			ch->GetDesc()->GetAccountTable().login,
			dwVnum,
			icount,
			lua_tostring(L,3),
...
#ifdef __BUFFI_SUPPORT__
	ALUA(pc_get_buffi_count)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		lua_pushnumber(L, ch ? BUFFI_MANAGER::Instance().SummonCount(ch) : 0);
		return 1;
	}
	ALUA(pc_set_buffi_name)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch || !lua_isstring(L, 1) || !BUFFI_MANAGER::Instance().SummonCount(ch))
		{
			lua_pushnumber(L, 0);
			return 1;
		}

		const char* szBuffiName = lua_tostring(L, 1);
		if (strlen(szBuffiName) < 2)
		{
			lua_pushnumber(L, 1);
			return 1;
		}
		if (!check_name(szBuffiName))
		{
			lua_pushnumber(L, 2);
			return 1;
		}

		BUFFI* buffi = BUFFI_MANAGER::Instance().GetBuffi(ch->GetPlayerID(), 0);
		if (!buffi)
		{
			lua_pushnumber(L, 0);
			return 1;
		}
		buffi->SetBuffiName(szBuffiName);
		lua_pushnumber(L, 99);
		return 1;
	}

	ALUA(pc_set_new_name)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		const char* szNewName = lua_tostring(L, 1);
		if (strlen(szNewName) < 2)
		{
			lua_pushnumber(L, 1);
			return 1;
		}
		if (!check_name(szNewName))
		{
			lua_pushnumber(L, 2);
			return 1;
		}
		ch->SetNewName(szNewName);
		lua_pushnumber(L, 99);
		return 1;
	}

	ALUA(pc_set_normal_name)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		const char* szNormalName = lua_tostring(L, 1);
		if (strlen(szNormalName) < 2)
		{
			lua_pushnumber(L, 1);
			return 1;
		}
		if (!check_name(szNormalName))
		{
			lua_pushnumber(L, 2);
			return 1;
		}
	...
	ALUA(pc_set_mount_name)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		const char* szMountName = lua_tostring(L, 1);
		if (strlen(szMountName) < 2)
		{
			lua_pushnumber(L, 1);
			return 1;
		}
		if (!check_name(szMountName))
		{
			lua_pushnumber(L, 2);
			return 1;
		}
		...
		ch->SetMountName(szMountName);

		LPITEM mount = ch->GetWear(WEAR_COSTUME_MOUNT_SKIN) ? ch->GetWear(WEAR_COSTUME_MOUNT_SKIN) : ch->GetWear(WEAR_COSTUME_MOUNT);
		if (mount && false == ch->IsRiding())
		{
			ch->HorseSummon(false);
			ch->HorseSummon(true, false, mount->GetValue(3));
		}
	...
	ALUA(pc_get_locale_codename)
	{
		LPCHARACTER lpChr = CQuestManager::instance().GetCurrentCharacterPtr();
		if (lpChr != nullptr && lpChr->GetDesc())
		{
			lua_pushstring(L, LC_CODE(lpChr->GetLanguage()));
			return 1;
		}
		lua_pushstring(L, LC_CODE(LOCALE_DEFAULT));
		return 1;
	}
	...
	ALUA(pc_get_locale)
	{
		...
		if (lpChr != nullptr && lpChr->GetDesc())
		{
			lua_pushnumber(L, lpChr->GetLanguage());
			return 1;
		}
		lua_pushnumber(L, LOCALE_DEFAULT);
		return 1;
	}
	...
	ALUA(pc_change_pin)
	{
		...
		if (pMsg->Get()->pSQLResult && pMsg->Get()->uiNumRows > 0)
		{
			...
			if (bResult)
			{
				...
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You have changed your PIN code."));
				lua_pushboolean(L, true);
				return 1;
			}
		}
	...
	int pc_get_okey_global_rank(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch)
			return 0;
	...
	int pc_get_okey_rund_rank(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch)
			return 0;
	...
	void RegisterPCFunctionTable()
	{
		luaL_reg pc_functions[] =
		{
			...
			{ "is_select_bonus_uriel", pc_is_select_bonus_uriel},
			{ "is_select_bonus_uriel_delay", pc_is_select_bonus_uriel_delay},
			{ "is_pet_delay", pc_is_pet_delay},
			{ "is_mount_delay", pc_is_mount_delay},
...
#if defined(__MINI_GAME_OKEY__)
			{ "get_okey_global_rank", pc_get_okey_global_rank },
			{ "get_okey_rund_rank", pc_get_okey_rund_rank },
#endif
...
#endif
#ifdef __BUFFI_SUPPORT__
			{ "get_buffi_count", pc_get_buffi_count },
			{ "set_buffi_name", pc_set_buffi_name },
			{ "set_new_name", pc_set_new_name },
			{ "set_normal_name", pc_set_normal_name },
			{ "set_mount_name", pc_set_mount_name },
#endif
#if defined(__GEM_SYSTEM__)
			{ "get_gem", pc_get_gem },
...
#endif
			{ "go_to_town", pc_go_to_town },
			{ "get_locale_codename", pc_get_locale_codename },
			{ "get_locale", pc_get_locale },
