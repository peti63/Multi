...
void DBManager::SendAuthLogin(LPDESC d)
{
	...
	strlcpy(ptod.szHWID, r.hwid, sizeof(ptod.szHWID));
	ptod.bLanguage = r.bLanguage;
#ifdef __MAINTENANCE__
	ptod.isTester = r.isTester;
#endif
		...
		case QID_AUTH_LOGIN:
			{
				...
				else
				{
...
#ifdef __MAINTENANCE__
					BYTE isTester;
					str_to_number(isTester, row[col++]);
#endif
					...
					// else if (!isTester)
					// {
					// 	LoginFailure(d, "BAKIM");
					// 	M2_DELETE(pinfo);
					// }
					// else if (CMaintenanceManager::Instance().GetGameMode() == GAME_MODE_MAINTENANCE)
					// {
					// 	LoginFailure(d, "BAKIM");
					// 	M2_DELETE(pinfo);
					// }				
					else if (CMaintenanceManager::Instance().GetGameMode() == GAME_MODE_MAINTENANCE && !isTester)
					{
						LoginFailure(d, "BAKIM");
						sys_log(0, "   BAKIM");
						M2_DELETE(pinfo);
					...
					else if (pinfo->version != GetGameVersion())
					{
						LoginFailure(d, "VERSION");
						sys_log(0, "version not same!");
						M2_DELETE(pinfo);
					}
#endif
					else
					{
						...
						auto aHWID = IsHWIDBlocked(pinfo->hwid);
						if (aHWID == HWID_STATUS_BLOCKED)
						{
							LoginFailure(d, "HWID_BAN");
							sys_log(0, "   HWID_BAN");
							M2_DELETE(pinfo);
							break;
						}
