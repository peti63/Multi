...
namespace quest
{
	...
	ALUA(dungeon_mission_notice)
	{
		...
		if(!pDungeon || pDungeon == NULL)
			return 0;
		if (pDungeon){
			pDungeon->MissionNotice(pDungeon->GetMapIndex(), lua_tostring(L, 2));
		}
		return 0;
	}
	...
	ALUA(dungeon_set_flag)
	{
		...
		else
		{
			...
			{
				sys_err("no dungeon !!! flag %s value %d", lua_tostring(L,1),int(lua_tonumber(L,2)));
			}
		}
		return 0;
	}
	...
	ALUA(dungeon_get_flag)
	{
		...
		{
			sys_err("no dungeon !!! flag %s value %d", lua_tostring(L,1));
			lua_pushnumber(L, 0);
		}
	...
	ALUA(dungeon_unique_set_immortality)
	{
		...
		if (pDungeon)
			pDungeon->UniqueSetImmortality(lua_tostring(L,1), (bool)lua_toboolean(L,2));
	...
	ALUA(dungeon_unique_set_affect)
	{
		...
		if (pDungeon)
			pDungeon->UniqueSetAffect(lua_tostring(L,1), (int)lua_tonumber(L,2), (int)lua_tonumber(L,3), (int)lua_tonumber(L,4));

		return 0;
	}

	ALUA(dungeon_unique_remove_affect)
	{
		if (!lua_isstring(L,1) || !lua_isnumber(L,2))
			return 0;
		...
		if (pDungeon)
			pDungeon->UniqueRemoveAffect(lua_tostring(L,1), (DWORD)lua_tonumber(L,2));
	...
	ALUA(dungeon_get_unique_presence)
	{
		if (!lua_isstring(L,1))
		{
			lua_pushboolean(L,0);
		...
		lua_pushboolean(L,pDungeon->GetUniquePresence(lua_tostring(L,1)));
		return 1;
	}

	ALUA(dungeon_get_unique_min_hp)
	{
		if (!lua_isstring(L,1))
		{
			lua_pushnumber(L,0);
		...
		if (pDungeon)
		{
			lua_pushnumber(L, pDungeon->GetUniqueMinHP(lua_tostring(L,1)));
			return 1;
		}
	...
	ALUA(dungeon_get_unique_max_hp)
	{
		if (!lua_isstring(L,1))
		{
			lua_pushnumber(L,0);
		...
		if (pDungeon)
		{
			lua_pushnumber(L, pDungeon->GetUniqueMaxHP(lua_tostring(L,1)));
			return 1;
		}
	...
	ALUA(dungeon_attack_unique)
	{
		...
		if (pDungeon)
		{
			pDungeon->AttackUniqueCharacter(pDungeon->GetMapIndex(), lua_tostring(L,1));
			return 1;
		}
	...
	ALUA(dungeon_count_members)
	{
		CQuestManager& q = CQuestManager::instance();
		LPDUNGEON pDungeon = q.GetCurrentDungeon();

		if (pDungeon)
			lua_pushnumber(L, pDungeon->CountMember());
		else
		{
	...
	struct FDungeonInfoFlagCompleted
	{
		...
		void operator () (LPENTITY ent)
		{
			...
			{
				LPCHARACTER pChar = static_cast<LPCHARACTER>(ent);
				if (pChar->IsPC() && !pChar->IsGM())
					pChar->SetQuestFlag(flag.c_str(), pChar->GetQuestFlag(flag.c_str()) + value);
					pChar->SetStatistics(STAT_TYPE_DUNGEONS, 1);
			}
		}
	};
	ALUA(dungeon_increase_completed)
	{
		CQuestManager& q = CQuestManager::instance();
		LPDUNGEON pDungeon = q.GetCurrentDungeon();
		if (!pDungeon)
	...
	struct FDungeonInfoFlagFastest
	{
		...
		void operator () (LPENTITY ent)
		{
			if (true == ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (pChar->IsPC() && !pChar->IsGM())
				{
					const int lastTime = pChar->GetQuestFlag(flag.c_str());
	...
	ALUA(dungeon_set_fastest)
	{
		...
		if (NULL != pSectree)
		{
			FDungeonInfoFlagFastest func(buf, (int)lua_tonumber(L, 2));
			pSectree->for_each(func);
		}
		return 0;
	}
	...
	ALUA(dungeon_quest_notice)
	{
		...
		BYTE bLocale = lpChr->GetLanguage();

		if (pDungeon){
			pDungeon->MissionNotice(pDungeon->GetMapIndex(), LC_QUEST(lua_tonumber(L, 2), bLocale));
		}
		return 0;
	}
	...
	void RegisterDungeonFunctionTable()
	{
		luaL_reg dungeon_functions[] =
		{
			...
			{ "unique_set_affect", dungeon_unique_set_affect},
			...
			{ "get_unique_presence", dungeon_get_unique_presence},
			{ "get_unique_min_hp",	dungeon_get_unique_min_hp },
			{ "get_unique_max_hp", dungeon_get_unique_max_hp },
			{ "attack_unique", dungeon_attack_unique },
			...
			{ "count_members",		dungeon_count_members	},
...
#ifdef ENABLE_UNIQUE_IMMORTALITY
			{ "set_immortality",				dungeon_unique_set_immortality},
#endif
#ifdef ENABLE_NEWSTUFF
			...
			{ "quest_notice", dungeon_quest_notice},
			{ NULL,				NULL			}
		};
