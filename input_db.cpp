...
// Function to write bytes to the vector
...
void CInputDB::PlayerLoad(LPDESC d, const char * data)
{
	...
	int allow_index = lMapIndex >= 10000 ? lMapIndex / 10000 : lMapIndex;
	if (!map_allow_find(allow_index))
	{			
		d->SetPhase(PHASE_CLOSE);
		return;
	}
...

	{
		...
		snprintf(buf, sizeof(buf), "%s %lld %d %d %ld %d",
			inet_ntoa(ch->GetDesc()->GetAddr().sin_addr), ch->GetGold(), ch->GetGem(), g_bChannel, ch->GetMapIndex(), ch->GetAlignment());
#else
		char buf[51];
...
void CInputDB::Boot(const char* data)
{
	...
	COfflineShopManager::Instance().LoadSearchLanguage();
#endif
...
void CInputDB::ItemLoad(LPDESC d, const char * c_pData)
{
	...
	while (it != v.end())
	{
		...
		{
			pos = ch->GetEmptySwitchInventory(item->GetSize());
		}
		else if (item->IsCostume())
		{
			pos = ch->GetEmptyCostumeInventory(item->GetSize());
		}
#endif
		...
		if (pos < 0)
		{
			...
			item->AddToGround2(ch->GetMapIndex(), coord, false, ch, 300);
