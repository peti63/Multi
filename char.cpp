...
void CHARACTER::Initialize()
{
	...
	memset(&m_lastSyncTime, 0, sizeof(m_lastSyncTime));
	m_iSyncHackCount = 0;
...
void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable * pTable, BYTE bItemCount)
{
	...
	for (BYTE i = 0; i < bItemCount; ++i)
	{
		...
		if (pkItem)
		{
			...
			if (item_table && (IS_SET(item_table->dwAntiFlags, ITEM_ANTIFLAG_GIVE | ITEM_ANTIFLAG_MYSHOP)) || pkItem->GetSocket(4) == 99)
			{
...
void CHARACTER::EncodeInsertPacket(LPENTITY entity)
{
...
#ifdef ENABLE_NEW_PET_SYSTEM
	if (IsPC() || m_bCharType == CHAR_TYPE_NPC || m_bCharType == CHAR_TYPE_HORSE || m_bCharType == CHAR_TYPE_PET)
#else
...
#endif
	{
		...
		if (false)
		{
			...
			if (GetEmpire() == ch->GetEmpire() || ch->GetGMLevel() > GM_PLAYER || m_bCharType == CHAR_TYPE_NPC || IsNewPet() || IsHorse())
			{
				goto show_all_info;
			}
			else
...
void CHARACTER::CreatePlayerProto(TPlayerTable & tab)
{
	...
	tab.iNormalInventory 		= Inventory_Point();
	tab.iSkillbookInventory		= Skill_Inventory_Point();
	tab.iMaterialInventory		= Material_Inventory_Point();
	tab.iStoneInventory			= Stone_Inventory_Point();
	tab.iChestInventory			= Chest_Inventory_Point();
	tab.iEnchantInventory		= Enchant_Inventory_Point();
	tab.iCostumeInventory		= Costume_Inventory_Point();
#endif
...
#endif
#ifdef NEW_STATISTICS
	memset(&tab.statsData, 0, sizeof(tab.statsData));
	if (m_statsData.size())
	{
		for (auto it = m_statsData.begin(); it != m_statsData.end(); it++)
		{
			if(it->second > 0)
				tab.statsData[it->first] = it->second;
		}
	}
#endif
}
...
#endif
	CHARACTER_MANAGER::Instance().CheckEnergyCrystal(this);
...
void CHARACTER::PointsPacket()
{
...
#ifdef ENABLE_EXTEND_INVEN_SYSTEM
	pack.points[POINT_INVENTORY]				= Inventory_Point();

	pack.points[POINT_SKILL_INVENTORY]			= Skill_Inventory_Point();
	pack.points[POINT_MATERIAL_INVENTORY]		= Material_Inventory_Point();
	pack.points[POINT_STONE_INVENTORY]			= Stone_Inventory_Point();
	pack.points[POINT_CHEST_INVENTORY]			= Chest_Inventory_Point();
	pack.points[POINT_ENCHANT_INVENTORY]		= Enchant_Inventory_Point();
	pack.points[POINT_COSTUME_INVENTORY]		= Costume_Inventory_Point();
#endif

#ifdef NEW_STATISTICS
	for (int i = 0; i < STAT_TYPE_MAX; i++)
		pack.points[POINT_TYPE_REFINE+i] = GetStatistics(i);
#endif
	...
	for (int i = POINT_ST; i < POINT_MAX_NUM; ++i)
	{
		...
		if (i == POINT_GEM)
		{
			pack.points[POINT_GEM] = GetGem(); // while in loop, if gaya == 168, set Gem and continue
			continue;
		}
#endif
...
#ifdef ENABLE_EXTEND_INVEN_SYSTEM
	Set_Inventory_Point(t->iNormalInventory);

	Set_Skill_Inventory_Point(t->iSkillbookInventory);
	Set_Material_Inventory_Point(t->iMaterialInventory);
	Set_Stone_Inventory_Point(t->iStoneInventory);
	Set_Chest_Inventory_Point(t->iChestInventory);
	Set_Enchant_Inventory_Point(t->iEnchantInventory);
	Set_Costume_Inventory_Point(t->iCostumeInventory);
#endif
...
#ifdef NEW_STATISTICS
	for (BYTE j = 0; j < STAT_TYPE_MAX; j++)
	{
		if (t->statsData[j] > 0)
			m_statsData.emplace(j, t->statsData[j]);
	}
#endif
...
int CHARACTER::GetMobElement(BYTE bElement) const
{
	if (bElement >= 0 && bElement < MOB_ELEMENT_MAX_NUM)
		return m_pkMobData ? m_pkMobData->m_table.cElements[bElement] : 0;
	return 0;
}	
...
WORD CHARACTER::GetMobAttackRange(DWORD victimRace) const
{
	switch (GetMobBattleType())
	{
		...
		default:
			...
			if (CShipDefenseManager::Instance().IsMinion(m_pkMobData->m_table.dwVnum) && victimRace == ShipDefense::EVNumHelper::SHIP_MAST)
...
void CHARACTER::ComputePoints()
{
...
#ifdef ENABLE_EXTEND_INVEN_SYSTEM
	long inventory			 	= Inventory_Point();

	long skill_inventory	 	= Skill_Inventory_Point();
	long material_inventory	 	= Material_Inventory_Point();
	long stone_inventory		= Stone_Inventory_Point();
	long chest_inventory		= Chest_Inventory_Point();
	long enchant_inventory		= Enchant_Inventory_Point();
	long costume_inventory		= Costume_Inventory_Point();
#endif
	...
	SetPoint(POINT_INVENTORY, inventory);

	SetPoint(POINT_SKILL_INVENTORY, skill_inventory);
	SetPoint(POINT_MATERIAL_INVENTORY, material_inventory);
	SetPoint(POINT_STONE_INVENTORY, stone_inventory);
	SetPoint(POINT_CHEST_INVENTORY, chest_inventory);
	SetPoint(POINT_ENCHANT_INVENTORY, enchant_inventory);
	SetPoint(POINT_COSTUME_INVENTORY, costume_inventory);

#ifdef NEW_STATISTICS
	for (int stats = 0; stats < STAT_TYPE_MAX; stats++)
		SetPoint(POINT_TYPE_REFINE+stats, GetStatistics(stats));
#endif
	...
	if (IsPC())
	{
		...
		{
			CSkillProto* pkSk2 = CSkillManager::instance().Get(SKILL_HUNTER);

			if (NULL != pkSk2)
			{
				pkSk2->SetPointVar("k", 1.0f * GetSkillPower(SKILL_HUNTER) / 100.0f);

				int skvalue2 = static_cast<int>(pkSk2->kPointPoly.Eval());
				PointChange(POINT_ATTBONUS_MONSTER, skvalue2);
				PointChange(POINT_ATTBONUS_HUMAN, skvalue2);
			}
		}

		{
			CSkillProto* pkSk3 = CSkillManager::instance().Get(SKILL_DURATION_TIME);

			if (NULL != pkSk3)
			{
				pkSk3->SetPointVar("k", 1.0f * GetSkillPower(SKILL_DURATION_TIME) / 100.0f);

				int skvalue3 = static_cast<int>(pkSk3->kPointPoly.Eval());
				PointChange(POINT_PARTY_BUFFER_BONUS, skvalue3);
			}
		}

		{
			// SKILL ALL P == SKILL_INSIGHT
			if (GetJob() == JOB_WARRIOR && GetSkillGroup() == 1)
			{
				if (GetSkillLevel(1) == 40 && GetSkillLevel(2) == 40 && GetSkillLevel(3) == 40 && GetSkillLevel(4) == 40 && GetSkillLevel(5) == 40 && GetSkillLevel(6) == 40)
				{
					PointChange(POINT_ATTBONUS_STONE, 8);
					PointChange(POINT_ATTBONUS_BOSS, 8);
				}

			}
			if (GetJob() == JOB_WARRIOR && GetSkillGroup() == 2)
			{
				if (GetSkillLevel(16) == 40 && GetSkillLevel(17) == 40 && GetSkillLevel(18) == 40 && GetSkillLevel(19) == 40 && GetSkillLevel(20) == 40 && GetSkillLevel(21) == 40)
				{
					PointChange(POINT_ATTBONUS_STONE, 8);
					PointChange(POINT_ATTBONUS_BOSS, 8);
				}
			}
			if (GetJob() == JOB_ASSASSIN && GetSkillGroup() == 1)
			{
				if (GetSkillLevel(31) == 40 && GetSkillLevel(32) == 40 && GetSkillLevel(33) == 40 && GetSkillLevel(34) == 40 && GetSkillLevel(35) == 40 && GetSkillLevel(36) == 40)
				{
					PointChange(POINT_ATTBONUS_STONE, 8);
					PointChange(POINT_ATTBONUS_BOSS, 8);
				}
			}
			if (GetJob() == JOB_ASSASSIN && GetSkillGroup() == 2)
			{
				if (GetSkillLevel(46) == 40 && GetSkillLevel(47) == 40 && GetSkillLevel(48) == 40 && GetSkillLevel(49) == 40 && GetSkillLevel(50) == 40 && GetSkillLevel(51) == 40)
				{
					PointChange(POINT_ATTBONUS_STONE, 8);
					PointChange(POINT_ATTBONUS_BOSS, 8);
				}
			...
			if (GetJob() == JOB_SURA && GetSkillGroup() == 1)
			{
				if (GetSkillLevel(61) == 40 && GetSkillLevel(62) == 40 && GetSkillLevel(63) == 40 && GetSkillLevel(64) == 40 && GetSkillLevel(65) == 40 && GetSkillLevel(66) == 40)
				{
					PointChange(POINT_ATTBONUS_STONE, 8);
					PointChange(POINT_ATTBONUS_BOSS, 8);
				}
			}
			if (GetJob() == JOB_SURA && GetSkillGroup() == 2)
			{
				if (GetSkillLevel(76) == 40 && GetSkillLevel(77) == 40 && GetSkillLevel(78) == 40 && GetSkillLevel(79) == 40 && GetSkillLevel(80) == 40 && GetSkillLevel(81) == 40)
				{
					PointChange(POINT_ATTBONUS_STONE, 8);
					PointChange(POINT_ATTBONUS_BOSS, 8);
				}
			}
			if (GetJob() == JOB_SHAMAN && GetSkillGroup() == 1)
			{
				if (GetSkillLevel(91) == 40 && GetSkillLevel(92) == 40 && GetSkillLevel(93) == 40 && GetSkillLevel(94) == 40 && GetSkillLevel(95) == 40 && GetSkillLevel(96) == 40)
				{
					PointChange(POINT_ATTBONUS_STONE, 8);
					PointChange(POINT_ATTBONUS_BOSS, 8);
				}
			}
			if (GetJob() == JOB_SHAMAN && GetSkillGroup() == 2)
			{
				if (GetSkillLevel(106) == 40 && GetSkillLevel(107) == 40 && GetSkillLevel(108) == 40 && GetSkillLevel(109) == 40 && GetSkillLevel(110) == 40 && GetSkillLevel(111) == 40)
				{
					PointChange(POINT_ATTBONUS_STONE, 8);
					PointChange(POINT_ATTBONUS_BOSS, 8);
				}
	...
	else
	{
		...
		SetPoint(POINT_MOV_SPEED, m_pkMobData->m_table.sMovingSpeed * 1.35);
	...
	if (!new_tree)
	{
		if (GetDesc())
		{
			...
			if (!new_tree)
			{
				...
				sys_err("cannot find old tree at %ld %ld (name: %s)", x, y, GetName());
			}
			// @fixme313 END
		}
		else
...
void CHARACTER::PointChange(BYTE type, int64_t amount, bool bAmount, bool bBroadcast)
{
	...
	switch (type)
	{
		...
		case POINT_INVENTORY:
			{
				const int64_t Envantertoplami = static_cast<int64_t>(Inventory_Point()) + static_cast<int64_t>(amount);
				if (Envantertoplami > INVENTORY_LOCK_COVER_COUNT)
				{
					sys_err("[POINT_INVEN ERROR]");
					return;
				}
				Set_Inventory_Point(Inventory_Point() + amount);
				val = Inventory_Point();
			}
			break;

		case POINT_SKILL_INVENTORY:
			{
				const int64_t Envantertoplami = static_cast<int64_t>(Skill_Inventory_Point()) + static_cast<int64_t>(amount);
				if (Envantertoplami > INVENTORY_LOCK_COVER_COUNT)
				{
					sys_err("[POINT_INVEN ERROR]");
					return;
				}
				Set_Skill_Inventory_Point(Skill_Inventory_Point() + amount);
				val = Skill_Inventory_Point();
				if (Inventory_Point() == 18 && Skill_Inventory_Point() == 18 && Material_Inventory_Point() == 18 && Stone_Inventory_Point() == 18 && Chest_Inventory_Point() == 18 && Enchant_Inventory_Point() == 18 && Costume_Inventory_Point() == 18)
					CHARACTER_MANAGER::Instance().SetRewardData(REWARD_INVEN_SLOT, GetName(), true);
			}
			break;
		case POINT_MATERIAL_INVENTORY:
			{
				const int64_t Envantertoplami = static_cast<int64_t>(Material_Inventory_Point()) + static_cast<int64_t>(amount);
				if (Envantertoplami > INVENTORY_LOCK_COVER_COUNT)
				{
					sys_err("[POINT_INVEN ERROR]");
					return;
				}
				Set_Material_Inventory_Point(Material_Inventory_Point() + amount);
				val = Material_Inventory_Point();
				if (Inventory_Point() == 18 && Skill_Inventory_Point() == 18 && Material_Inventory_Point() == 18 && Stone_Inventory_Point() == 18 && Chest_Inventory_Point() == 18 && Enchant_Inventory_Point() == 18 && Costume_Inventory_Point() == 18)
					CHARACTER_MANAGER::Instance().SetRewardData(REWARD_INVEN_SLOT, GetName(), true);
			}
			break;
		case POINT_STONE_INVENTORY:
			{
				const int64_t Envantertoplami = static_cast<int64_t>(Stone_Inventory_Point()) + static_cast<int64_t>(amount);
				if (Envantertoplami > INVENTORY_LOCK_COVER_COUNT)
				{
					sys_err("[POINT_INVEN ERROR]");
					return;
				}
				Set_Stone_Inventory_Point(Stone_Inventory_Point() + amount);
				val = Stone_Inventory_Point();
				if (Inventory_Point() == 18 && Skill_Inventory_Point() == 18 && Material_Inventory_Point() == 18 && Stone_Inventory_Point() == 18 && Chest_Inventory_Point() == 18 && Enchant_Inventory_Point() == 18 && Costume_Inventory_Point() == 18)
					CHARACTER_MANAGER::Instance().SetRewardData(REWARD_INVEN_SLOT, GetName(), true);
			}
			break;
		case POINT_CHEST_INVENTORY:
			{
				const int64_t Envantertoplami = static_cast<int64_t>(Chest_Inventory_Point()) + static_cast<int64_t>(amount);
				if (Envantertoplami > INVENTORY_LOCK_COVER_COUNT)
				{
					sys_err("[POINT_INVEN ERROR]");
					return;
				}
				Set_Chest_Inventory_Point(Chest_Inventory_Point() + amount);
				val = Chest_Inventory_Point();
				if (Inventory_Point() == 18 && Skill_Inventory_Point() == 18 && Material_Inventory_Point() == 18 && Stone_Inventory_Point() == 18 && Chest_Inventory_Point() == 18 && Enchant_Inventory_Point() == 18 && Costume_Inventory_Point() == 18)
					CHARACTER_MANAGER::Instance().SetRewardData(REWARD_INVEN_SLOT, GetName(), true);
			}
			break;
		case POINT_ENCHANT_INVENTORY:
			{
				const int64_t Envantertoplami = static_cast<int64_t>(Enchant_Inventory_Point()) + static_cast<int64_t>(amount);
				if (Envantertoplami > INVENTORY_LOCK_COVER_COUNT)
				{
					sys_err("[POINT_INVEN ERROR]");
					return;
				}
				Set_Enchant_Inventory_Point(Enchant_Inventory_Point() + amount);
				val = Enchant_Inventory_Point();
				if (Inventory_Point() == 18 && Skill_Inventory_Point() == 18 && Material_Inventory_Point() == 18 && Stone_Inventory_Point() == 18 && Chest_Inventory_Point() == 18 && Enchant_Inventory_Point() == 18 && Costume_Inventory_Point() == 18)
					CHARACTER_MANAGER::Instance().SetRewardData(REWARD_INVEN_SLOT, GetName(), true);
			}
			break;
		case POINT_COSTUME_INVENTORY:
			{
				const int64_t Envantertoplami = static_cast<int64_t>(Costume_Inventory_Point()) + static_cast<int64_t>(amount);
				if (Envantertoplami > INVENTORY_LOCK_COVER_COUNT)
				{
					sys_err("[POINT_INVEN ERROR]");
					return;
				}
				Set_Costume_Inventory_Point(Costume_Inventory_Point() + amount);
				val = Costume_Inventory_Point();
				if (Inventory_Point() == 18 && Skill_Inventory_Point() == 18 && Material_Inventory_Point() == 18 && Stone_Inventory_Point() == 18 && Chest_Inventory_Point() == 18 && Enchant_Inventory_Point() == 18 && Costume_Inventory_Point() == 18)
					CHARACTER_MANAGER::Instance().SetRewardData(REWARD_INVEN_SLOT, GetName(), true);
			}
			break;
		...
		case POINT_GEM:
			{
				const long long nTotalGem = static_cast<long long>(GetGem()) + static_cast<long long>(amount);
...
#endif
#ifdef NEW_STATISTICS
		case POINT_TYPE_REFINE:
		case POINT_TYPE_CRAFT:
		case POINT_TYPE_FISH:
		case POINT_TYPE_DUNGEONS:
		case POINT_TYPE_SHOUT:
		case POINT_TYPE_ADD_SOCKET:
		case POINT_TYPE_CHESTS:
		case POINT_TYPE_TRADE:
		case POINT_TYPE_MONSTERS:
		case POINT_TYPE_BOSSES:
		case POINT_TYPE_STONES:
		case POINT_TYPE_PC_A:
		case POINT_TYPE_PC_B:
		case POINT_TYPE_PC_C:
		case POINT_TYPE_DUEL:
		case POINT_TYPE_DEAD:
		case POINT_TYPE_GUILD_WAR:
		case POINT_TYPE_GUILD_KILLS:
		case POINT_TYPE_GUILD_FLAGS:
		case POINT_TYPE_BATTLEPASS:
		case POINT_TYPE_SKILLBOOKS:
		case POINT_TYPE_HUNTING_MISSIONS:
			{
				SetPointStatistics(type-POINT_TYPE_REFINE, amount);
				val = GetStatistics(type-POINT_TYPE_REFINE);
			}
			break;
		case POINT_TYPE_DMG_STONES:
		case POINT_TYPE_DMG_HIT_PC:
		case POINT_TYPE_DMG_SKILL_PC:
			{
				SetPointStatistics(type-POINT_TYPE_REFINE, amount);
				val = GetStatistics(type-POINT_TYPE_REFINE);
			}
		...
		case POINT_ATT_SPEED:
		// {
		// 	SetPoint(type, (GetPoint(type) + amount) > 170 ? 170 : (GetPoint(type) + amount));
		// 	val = GetPoint(type);
		// 	BroadcastTargetPacket();
		// } break;
		...
		case POINT_CRITICAL_DAMAGE_PVM:
...
void CHARACTER::ApplyPoint(BYTE bApplyType, int iVal)
{
	switch (bApplyType)
	{
		...
		case APPLY_CRITICAL_DAMAGE_PVM:
		case APPLY_RESIST_RACES:
		case APPLY_ATTBONUS_STONE:
		case APPLY_ATTBONUS_DUNGEON:

		case APPLY_PARTY_ATTACKER_BONUS:
		case APPLY_PARTY_HASTE_BONUS:
		case APPLY_PARTY_TANKER_BONUS:
		case APPLY_PARTY_DEFENDER_BONUS:
		case APPLY_PARTY_BUFFER_BONUS:
		case APPLY_PARTY_SKILL_MASTER_BONUS:
#endif
			PointChange(aApplyInfo[bApplyType].bPointType, iVal);
			break;
...
bool CHARACTER::SetSyncOwner(LPCHARACTER ch, bool bRemoveFromList)
{
	...
	else
	{
		...
		if (m_pkChrSyncOwner != ch)
		{
			...
			// Seteaz? timpul ultimei sincroniz?ri la momentul curent
			SetLastSyncTime(std::chrono::steady_clock::now());
...
bool CHARACTER::WarpSet(long x, long y, long lPrivateMapIndex, BYTE newChannel)
{
	if (!IsPC())
		return false;
	...
	if (!CMapLocation::instance().Get(x, y, newMapIndex, newAddr, newPort, newChannel))
		return false;
	...
	{
		if (lPrivateMapIndex / 10000 != newMapIndex)
		{
			sys_err("Invalid map index %d, must be child of %d", lPrivateMapIndex, newMapIndex);
			return false;
		}

		newMapIndex = lPrivateMapIndex;
	}
	...
	m_lWarpMapIndex = newMapIndex;
	m_posWarp.x = x;
	m_posWarp.y = y;

	sys_log(0, "WarpSet %s %d %d current map %d target map %d", GetName(), x, y, GetMapIndex(), newMapIndex);
...
bool CHARACTER::Follow(LPCHARACTER pkChr, float fMinDistance)
{
	...
	if (pkChr->IsState(pkChr->m_stateMove) &&
		...
		false == IsPet() && false == IsNewPet() && false == IsHorse() && false == IsBuffi())
	{
...
bool CHARACTER::PreventTradeWindow(int flags, bool except) const
{
	...
	if ((except && !(WND_SHOP & flags)) || (!except && (WND_SHOP & flags)))
	{
		if (GetShop())
			return true;
	}
	...
	if ((except && !(WND_MYSHOP & flags)) || (!except && (WND_MYSHOP & flags)))
		if (GetMyShop() || GetOfflineShopPrevent())
			return true;

	if ((except && !(WND_SHOPOWNER & flags)) || (!except && (WND_SHOPOWNER & flags)))
	{
		if (GetShopOwner() || GetOfflineShopPanelPrevent())
			return true;
	}
	...
	if ((except && !(WND_MALLBOX & flags)) || (!except && (WND_MALLBOX & flags)))
		if (IsOpenMall())
			return true;
...
EVENTFUNC(check_speedhack_event)
{
	...
	{
		// write hack log
		LogManager::instance().SpeedHackLog(ch->GetPlayerID(), ch->GetMapIndex(), ch->GetX(), ch->GetY(), ch->m_speed_hack_count);
...
void CHARACTER::Open67Attr()
{
	...
	if (PreventTradeWindow(WND_ATTR67ADD, true))
	{
...
void CHARACTER::SetNewName(const char* newName)
{
	char filename[124];
	snprintf(filename, sizeof(filename), "data/pet_name/new/%u", GetPlayerID());
...
void CHARACTER::SetNormalName(const char* newName)
{
	if (!check_name(newName))
		return;

	char filename[124];
	snprintf(filename, sizeof(filename), "data/pet_name/normal/%u", GetPlayerID());
	FILE* fp = fopen(filename, "w+");
	if (fp)
	{
		fprintf(fp, "%s", newName);
...
void CHARACTER::SetMountName(const char* newName)
{
	char filename[124];	
	snprintf(filename, sizeof(filename), "data/mount_name/%u", GetPlayerID());
	FILE* fp = fopen(filename, "w+");
	if (fp)
	{
		fprintf(fp, "%s", newName);
...
void CHARACTER::RefineAcceMaterials()
{
	...
	if (bCan == 1)
	{
		...
		if (bSucces)
		{
			...
			if (pkItem->GetSocket(0) == 25)
				CHARACTER_MANAGER::Instance().SetRewardData(REWARD_ACCE,GetName(), true);
...
bool CHARACTER::CanAddItemShop()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_OLD_TIME %d"), GetProtectTime("offlineshop.additem") - now);
		return true;
	}
...
bool CHARACTER::CanDestroyShop()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_OLD_TIME %d"), GetProtectTime("offlineshop.destroy") - now);
		return true;
	}
...
bool CHARACTER::CanRemoveItemShop()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_OLD_TIME %d"), GetProtectTime("offlineshop.remove")-now);
		return true;
	}
...
bool CHARACTER::CanCreateShop()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_OLD_TIME %d"), GetProtectTime("offlineshop.create")-time);
		return true;
	}
	...
	switch (GetMapIndex())
	{
		...
		case 41:
			{
				if (GetGold() < 2000000)
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_CREATE_YANG %d"),2000000);
					return true;
				}
				else if (GetDungeon())
				{
		...
		default:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_CANT_CREATE_OTHER_MAPS"));
			return true;
	}
}
...
bool CHARACTER::CheckPremiumStateMap()
{
	...
	auto m_data = GetOfflineShop()->m_data;

	if (GetMapIndex() != m_data.mapindex && FindAffect(AFFECT_DECORATION))
		return true;

	if (GetMapIndex() == m_data.mapindex)
		return true;
...
bool CHARACTER::CanRemoveLogShop()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_OLD_TIME %d"), GetProtectTime("offlineshop.log") - time);
		return true;
	}
...
bool CHARACTER::CanWithdrawMoney()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_OLD_TIME %d"), GetProtectTime("offlineshop.money") - time);
		return true;
	}
...
bool CHARACTER::CanOpenShopPanel()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_OLD_TIME %d"), GetProtectTime("offlineshop.panel") - time);
		return true;
	}
...
bool CHARACTER::CanOpenOfflineShop()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_OLD_TIME %d"), GetProtectTime("offlineshop.offlineshop") - time);
		return true;
	}
...
bool CHARACTER::CanChangeTitle()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_OLD_TIME %d"), GetProtectTime("offlineshop.title") - time);
		return true;
	}
...
bool CHARACTER::CanBuyItemOfflineShop()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_OLD_TIME %d"), GetProtectTime("offlineshop.buy") - time);
		return true;
	}
...
bool CHARACTER::CanChangeDecoration()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_OLD_TIME %d"), GetProtectTime("offlineshop.decoration") - time);
		return true;
	}
...
bool CHARACTER::CanGetBackItems()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_OLD_TIME %d"), GetProtectTime("offlineshop.back") - time);
		return true;
	}
...
bool CHARACTER::CanAddTimeShop()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_OLD_TIME %d"), GetProtectTime("offlineshop.time") - time);
		return true;
	}
...
bool CHARACTER::CanMoveItemOfflineShop()
{
	if (m_pkTimedEvent)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_BUG_WITH_TIMED_EVENT"));
...
bool CHARACTER::CanChangePriceShop()
{
	if (m_pkTimedEvent)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_BUG_WITH_TIMED_EVENT"));
...
bool CHARACTER::CanTakeOutAllItemsOfflineShop()
{
	if (m_pkTimedEvent)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_BUG_WITH_TIMED_EVENT"));
...
bool CHARACTER::CanTeleportOfflineShop()
{
	if (m_pkTimedEvent)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_BUG_WITH_TIMED_EVENT"));
...
bool CHARACTER::CanSearch()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_OLD_TIME %d"), GetProtectTime("offlineshop.search")-now);
		return true;
	}
	SetProtectTime("offlineshop.search", now + 1);
	return false;
}
bool CHARACTER::IsLookingSearchItem(DWORD itemID, bool remove, bool sendCommand)
{
	auto it = std::find(m_vecSearchLooking.begin(), m_vecSearchLooking.end(), itemID);
	if (it != m_vecSearchLooking.end())
	{
		auto currentIt = std::find(m_vecCurrentLooking.begin(), m_vecCurrentLooking.end(), itemID);
		if(sendCommand && GetDesc() && currentIt != m_vecCurrentLooking.end())
			ChatPacket(CHAT_TYPE_COMMAND, "OfflineShopBuyed %u", itemID);
		if (remove)
		{
			m_vecSearchLooking.erase(it);
			if(currentIt != m_vecCurrentLooking.end())
				m_vecCurrentLooking.erase(currentIt);
		}
		return true;
	}
	return false;
}
void CHARACTER::SetLookingSearch(const std::vector<DWORD> dataVector)
{
	m_vecCurrentLooking.clear();
	m_vecSearchLooking.clear();
	for (int i = 0; i < dataVector.size(); ++i)
		m_vecSearchLooking.emplace_back(dataVector[i]);
}
int CHARACTER::GetTotalPageCount()
{
	int pageCount = 1;
	if (m_vecSearchLooking.size() > SEARCH_MAX_ITEM_PER_PAGE)
	{
		pageCount = m_vecSearchLooking.size() / SEARCH_MAX_ITEM_PER_PAGE;
		if (pageCount < 1)
			pageCount = 1;
		if (m_vecSearchLooking.size() - (pageCount * SEARCH_MAX_ITEM_PER_PAGE) > 0)
			pageCount += 1;
	}
	return pageCount;
}
void CHARACTER::GetPageItems(int pageIdx, std::vector<DWORD>& m_ItemList)
{
	m_ItemList.clear();
	m_vecCurrentLooking.clear();
	const int start = (pageIdx - 1) * SEARCH_MAX_ITEM_PER_PAGE;
	const int end = ((pageIdx - 1) * SEARCH_MAX_ITEM_PER_PAGE) + SEARCH_MAX_ITEM_PER_PAGE;
	for (int i = start; i < end; ++i)
	{
		if (i >= m_vecSearchLooking.size())
			break;
		m_ItemList.emplace_back(m_vecSearchLooking[i]);
		m_vecCurrentLooking.emplace_back(m_vecSearchLooking[i]);
	}
}
#endif
...
EVENTFUNC(brave_cape_event)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Brave affect is done loop event closed."));
		ch->m_pkBraveCapeEvent = NULL;
		return 0;
	}

	if (ch->GetLevel() > 30 && ch->GetLevel() < 61) {
		if (ch->GetGold() < LOW_COST_CAPE) {
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You don't have enough money."));
			return 0;
		}
	}
	if (ch->GetLevel() > 60 && ch->GetLevel() < 91) {
		if (ch->GetGold() < MEDIUM_COST_CAPE) {
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You don't have enough money."));
			return 0;
		}
	}
	if (ch->GetLevel() > 90 && ch->GetLevel() < 121) {
		if (ch->GetGold() < HIGH_COST_CAPE) {
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You don't have enough money."));
			return 0;
		}
	}
	...
	if (ch->GetLevel() > 30 && ch->GetLevel() < 61) {
		ch->PointChange(POINT_GOLD, -LOW_COST_CAPE, true);
	}
	if (ch->GetLevel() > 60 && ch->GetLevel() < 91) {
		ch->PointChange(POINT_GOLD, -MEDIUM_COST_CAPE, true);
	}
	if (ch->GetLevel() > 90 && ch->GetLevel() < 121) {
		ch->PointChange(POINT_GOLD, -HIGH_COST_CAPE, true);
	}
#endif
#ifdef ENABLE_AGGREGATE_MONSTER_EFFECT
	ch->EffectPacket(SE_AGGREGATE_MONSTER_EFFECT);
#endif
...
void CHARACTER::GetBraveCapeCMDCompare(const char* c_pszArgument)
{
	...
	else if (vecArgs[1] == "active")
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You don't has brave cape affect."));
			return;
		}
		if (m_pkBraveCapeEvent)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Already brave cape working.."));
			return;
		}
		// const std::vector<WORD> m_dwBlockedMapIdx = {};
		...
		if (loopTime < 2 || loopTime > 60 || eventRange < 1000 || eventRange > 5500)
			return;
		...
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Brave cape event started.."));
	...
	else if (vecArgs[1] == "deactive")
	{
		if (!m_pkBraveCapeEvent)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You don't has brave cape loop event!"));
			return;
		}
		...
		event_cancel(&m_pkBraveCapeEvent);
		m_pkBraveCapeEvent = NULL;
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Brave cape event closed.."));
...
{
	char filename[124];
	snprintf(filename, sizeof(filename), "data/missionbook/players/%u", GetPlayerID());
	FILE* fp = fopen(filename, "w+");
	if (fp)
	{
...
void CHARACTER::SortInventoryItems()
{
	...
	if (GetSortInventoryPulse() > get_global_time())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You have to wait %d s! Try again later."), GetSortInventoryPulse() - get_global_time());
		return;
	}
	...
	std::vector<LPITEM> vec_pkItemsWithSocket99, vec_pkItemsWithoutSocket99;
	LPITEM pkItem;

	for (WORD i = 0; i < INVENTORY_MAX_NUM; ++i)
	{
		if ((pkItem = GetInventoryItem(i)) != nullptr)
		{
			if (pkItem->GetSocket(4) == 99) {
				vec_pkItemsWithSocket99.emplace_back(pkItem);
			} else {
				vec_pkItemsWithoutSocket99.emplace_back(pkItem);
			}
	...
	std::sort(vec_pkItemsWithSocket99.begin(), vec_pkItemsWithSocket99.end(), SortItems);
	std::sort(vec_pkItemsWithoutSocket99.begin(), vec_pkItemsWithoutSocket99.end(), SortItems);

	// Logica de procesare a obiectelor cu Socket 4 == 99
	for (const auto& item : vec_pkItemsWithSocket99)
	{
		ProcessSortedItem(item); // Presupun?nd c? exist? o func?ie 'ProcessSortedItem'
	}

	// Logica de procesare a obiectelor f?r? Socket 4 == 99
	for (const auto& item : vec_pkItemsWithoutSocket99)
	{
		ProcessSortedItem(item); // Aceea?i func?ie poate fi folosit?, dac? este adecvat?
	}
...
void CHARACTER::ProcessSortedItem(LPITEM item)
{
	...
	{
		auto nItem = AutoGiveItemType(item->GetVnum(), INVENTORY, item->GetCount(), -1, false);
		nItem->SetSocket(4, item->GetSocket(4), false);
		M2_DESTROY_ITEM(item);
	}
	else
		AutoGiveItemType(item, INVENTORY, false);
}
...
void CHARACTER::SortSpecialInventoryItems(BYTE type)
{
	...
	BYTE inventory_type = DetermineInventoryType(type);

	CollectItems(type, vec_pkItemsWithSocket99, vec_pkItemsWithoutSocket99);

	std::sort(vec_pkItemsWithSocket99.begin(), vec_pkItemsWithSocket99.end(), SortItems);
	std::sort(vec_pkItemsWithoutSocket99.begin(), vec_pkItemsWithoutSocket99.end(), SortItems);

	ProcessSortedItems(vec_pkItemsWithSocket99, inventory_type);
	ProcessSortedItems(vec_pkItemsWithoutSocket99, inventory_type);
...
BYTE CHARACTER::DetermineInventoryType(BYTE type)
{
	switch (type)
	{
		case 1: return UPGRADE_ITEMS_INVENTORY;
		case 2: return STONE_INVENTORY;
		case 3: return GIFT_BOX_INVENTORY;
		case 4: return SWITCH_INVENTORY;
		case 5: return COSTUME_INVENTORY;
		default: return SKILL_BOOK_INVENTORY;
	}
}

void CHARACTER::CollectItems(BYTE type, std::vector<LPITEM>& withSocket, std::vector<LPITEM>& withoutSocket)
{
	LPITEM pkItem;
	WORD start, end;
	GetInventoryRange(type, start, end);

	for (WORD i = start; i < end; ++i)
	{
		pkItem = GetSpecialInventoryItem(type, i);
		if (pkItem)
		{
			if (pkItem->GetSocket(4) == 99) {
				withSocket.emplace_back(pkItem);
			} else {
				withoutSocket.emplace_back(pkItem);
			}
			pkItem->RemoveFromCharacter();
		}
	}
...
void CHARACTER::GetInventoryRange(BYTE type, WORD& start, WORD& end)
{
	switch (type)
	{
		case 1: // UPGRADE_ITEMS_INVENTORY
			start = UPGRADE_ITEMS_INVENTORY_SLOT_START;
			end = UPGRADE_ITEMS_INVENTORY_SLOT_END;
			break;
		case 2: // STONE_INVENTORY
			start = STONE_INVENTORY_SLOT_START;
			end = STONE_INVENTORY_SLOT_END;
			break;
		case 3: // GIFT_BOX_INVENTORY
			start = GIFT_BOX_INVENTORY_SLOT_START;
			end = GIFT_BOX_INVENTORY_SLOT_END;
			break;
		case 4: // SWITCH_INVENTORY
			start = SWITCH_INVENTORY_SLOT_START;
			end = SWITCH_INVENTORY_SLOT_END;
			break;
		case 5: // COSTUME_INVENTORY
			start = COSTUME_INVENTORY_SLOT_START;
			end = COSTUME_INVENTORY_SLOT_END;
			break;
		default: // SKILL_BOOK_INVENTORY
...
LPITEM CHARACTER::GetSpecialInventoryItem(BYTE type, WORD index)
{
	switch (type)
	{
		case 1: // UPGRADE_ITEMS_INVENTORY
			return GetUpgradeItemsInventoryItem(index);
		case 2: // STONE_INVENTORY
			return GetStoneInventoryItem(index);
		case 3: // GIFT_BOX_INVENTORY
			return GetGiftBoxInventoryItem(index);
		case 4: // SWITCH_INVENTORY
			return GetSwitchInventoryItem(index);
		case 5: // COSTUME_INVENTORY
			return GetCostumeInventoryItem(index);
		default: // SKILL_BOOK_INVENTORY
			return GetSkillBookInventoryItem(index);
	}
}

void CHARACTER::ProcessSortedItems(const std::vector<LPITEM>& items, BYTE inventory_type)
{
	for (const auto& item : items)
	{
		...
		if (itemTable->dwFlags & ITEM_FLAG_STACKABLE && itemTable->bType != ITEM_BLEND)
		{
			auto nItem = AutoGiveItemType(item->GetVnum(), inventory_type, item->GetCount(), -1, false);
			nItem->SetSocket(4, item->GetSocket(4), false);
			M2_DESTROY_ITEM(item);
		}
		else
			AutoGiveItemType(item, inventory_type, false);
	}
}
...
{
	ChatPacket(CHAT_TYPE_COMMAND, "AntiExp %d", 1);
	SetQuestFlag("anti_exp.freeze", 1);
	return;
}

void CHARACTER::UnfreezeEXP()
{
	ChatPacket(CHAT_TYPE_COMMAND, "AntiExp %d", 0);
	SetQuestFlag("anti_exp.freeze", 0);
	return;
}
#endif

#ifdef ENABLE_EXTEND_INVEN_SYSTEM
bool CHARACTER::Update_Inventory(INT type)
{
	switch (type)
	{
		case 0:
		{
			if (Inventory_Point() >= INVENTORY_LOCK_COVER_COUNT) return false;

			int needkey = 10;
			int keycount = CountSpecifyItem(72320);
			if (keycount >= needkey) {
				int willdelete = 72320;
				while (needkey) {
			...
			PointChange(POINT_INVENTORY, 1, false);
			return true;
		...
		case 1:
		{
			if (Skill_Inventory_Point() >= INVENTORY_LOCK_COVER_COUNT) return false;
			int needkey = 10;
			int keycount = CountSpecifyItem(72320);
			if (keycount >= needkey) {
				int willdelete = 72320;
			...
			PointChange(POINT_SKILL_INVENTORY, 1, false);
			return true;
		}
		break;
		case 2:
		{
			if (Material_Inventory_Point() >= INVENTORY_LOCK_COVER_COUNT) return false;
			int needkey = 10;
			int keycount = CountSpecifyItem(72320);
			if (keycount >= needkey) {
				int willdelete = 72320;
			...
			PointChange(POINT_MATERIAL_INVENTORY, 1, false);
			return true;
		...
		case 3:
		{
			if (Stone_Inventory_Point() >= INVENTORY_LOCK_COVER_COUNT) return false;
			int needkey = 10;
			int keycount = CountSpecifyItem(72320);
			if (keycount >= needkey) {
				int willdelete = 72320;
			...
			PointChange(POINT_STONE_INVENTORY, 1, false);
			return true;
		}
		break;
		case 4:
		{
			if (Chest_Inventory_Point() >= INVENTORY_LOCK_COVER_COUNT) return false;
			int needkey = 10;
			int keycount = CountSpecifyItem(72320);
			if (keycount >= needkey) {
				int willdelete = 72320;
			...
			PointChange(POINT_CHEST_INVENTORY, 1, false);
			return true;
		}
		break;
		case 5:
		{
			if (Enchant_Inventory_Point() >= INVENTORY_LOCK_COVER_COUNT) return false;
			int needkey = 10;
			int keycount = CountSpecifyItem(72320);
			if (keycount >= needkey) {
				int willdelete = 72320;
			...
			PointChange(POINT_ENCHANT_INVENTORY, 1, false);
			return true;
		}
		break;
		case 6:
		{
			if (Costume_Inventory_Point() >= INVENTORY_LOCK_COVER_COUNT) return false;
			int needkey = 10;
			int keycount = CountSpecifyItem(72320);
			if (keycount >= needkey) {
				int willdelete = 72320;
			...
			PointChange(POINT_COSTUME_INVENTORY, 1, false);
			return true;
		}
		break;
		default:
...
#ifdef __SASH_SKIN__
bool CHARACTER::UpgradeDreamSoul(LPITEM item, bool printMsg)
{
	if (item)
	{
		if (!item->IsDreamSoul())
		{
			if (printMsg)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can't upgrade this item!"));
			return false;
		}

		const auto it = dreamSoulRefineData.find(item->GetVnum());
		if (it == dreamSoulRefineData.end())
		{
			if (printMsg)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can't upgrade this item!"));
			return false;
		...
		const BYTE window_type = item->GetWindow();
		const WORD slot_idx = item->GetCell();

		const dream_soul& soulData = it->second;

		if (GetGold() < soulData.cost)
		{
			if (printMsg)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You don't have enought gold!"));
			return false;
		}

		for (const auto& [itemIdx, itemCount] : soulData.needItems)
		{
			if (CountSpecifyItem(itemIdx, slot_idx) < itemCount)
			{
				if(printMsg)
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You don't have enought item!"));
				return false;
			}
		}

		for (const auto& [itemIdx, itemCount] : soulData.needItems)
			RemoveSpecifyItem(itemIdx, itemCount, slot_idx);

		PointChange(POINT_GOLD, -static_cast<int64_t>(soulData.cost));

		if (number(1, 99) < soulData.lucky)
		{
			LPITEM newItem = ITEM_MANAGER::Instance().CreateItem(soulData.nextItem);
			if (!newItem)
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Something went wrong when create new item!"));
				return false;
			}
			item->SetCount(0);
			newItem->AddToCharacter(this, TItemPos(window_type, slot_idx));
			newItem->Save();

			if(printMsg)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Upgrade Successfully"));
		}
		else
		{
			if (printMsg)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Upgrade failed"));
		}
		ChatPacket(CHAT_TYPE_COMMAND, "SashSkinUpgradeRefresh %u %u", window_type, slot_idx);
	}
	return true;
}
#endif
...
void CHARACTER::SetStatistics(BYTE typeIndex, int64_t value)
{
	PointChange(typeIndex + POINT_TYPE_REFINE, value);
}

void CHARACTER::SetPointStatistics(BYTE typeIndex, int64_t value)
{
	int64_t oldValue = GetStatistics(typeIndex);
	auto it = m_statsData.find(typeIndex);

	if (typeIndex == STAT_TYPE_DMG_STONES || typeIndex == STAT_TYPE_DMG_HIT_PC || typeIndex == STAT_TYPE_DMG_SKILL_PC)
	{
		if (oldValue >= value)
			return;

		if (it != m_statsData.end())
			it->second = value;
		else
			m_statsData.emplace(typeIndex, value);
	}
	else
	{
		if (it != m_statsData.end())
			it->second = oldValue + value;
		else
			m_statsData.emplace(typeIndex, oldValue + value);
	}
}

int64_t CHARACTER::GetStatistics(BYTE typeIndex)
{
	auto it = m_statsData.find(typeIndex);
