...
#define _AUTO_DETECT_INTERNAL_IP_
...
bool g_bAntiExpRing = true;
#endif

#ifdef __CLIENT_VERSION__
DWORD version = 1000011;
...
std::string	g_stAdminPagePassword = "BUMJAGALAKAEFIERBINTACAVARA";
...
void config_init(const string& st_localeServiceName)
{
...
#if defined(__CLIENT_VERSION__) && !defined(__WIN32__)
	char buf[256];

	st_configFileName = "data/VERSION";
	FILE* fpOnlyForVersion;
	if (!(fpOnlyForVersion = fopen(st_configFileName.c_str(), "r")))
	{
		fprintf(stderr, "Can not open [%s]\n", st_configFileName.c_str());
		exit(1);
	}
	while (fgets(buf, 256, fpOnlyForVersion))
	{
		if(strlen(buf))
		{
			if(str_to_number(version, buf))
				break;
		}
	}
	fclose(fpOnlyForVersion);
	st_configFileName = "";
#endif
	...
	sys_err("%s", st_configFileName.c_str());
