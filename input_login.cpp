		...
		BYTE bDex)
{
	...
	{
		int randomlobby = number(1,11);
		table->x		= CREATE_START_X(bEmpire, randomlobby);
		table->y		= CREATE_START_Y(bEmpire, randomlobby);
	}
	else
	{
		int randomnormal = number(1,4);
		table->x		= CREATE_START_X(bEmpire, randomnormal);
		table->y		= CREATE_START_Y(bEmpire, randomnormal);
	}
#endif
	table->z	= 0;
	table->dir	= 0;
...
void CInputLogin::Entergame(LPDESC d, const char * data)
{
	...
	if (ch->HasFrozenEXP())
		ch->ChatPacket(CHAT_TYPE_COMMAND, "AntiExp %d", 1);

#ifdef __PENDANT__
	// if (!ch->GetWear(WEAR_PENDANT_SOUL_FIRST))
	// {
	// 	LPITEM item = ITEM_MANAGER::Instance().CreateItem(10790);
	// 	if (item)
	// 	{
	...
	// {
	// 	LPITEM item = ITEM_MANAGER::Instance().CreateItem(10791);
	// 	if (item)
	// 	{
	// 		ch->AutoGiveItem(item);
	// 		ch->EquipItem(item);
	// 	}
	// }
	ch->ChatPacket(CHAT_TYPE_COMMAND, "SetPendantPageIdx %d %d", ch->GetQuestFlag("pendant.page"), ch->GetQuestFlag("pendant.opened"));
#endif
...
#endif
	CHARACTER_MANAGER::Instance().CheckEnergyCrystal(ch);
}
